from admin_panel.models import Task, SiteData  # Import SiteData at the top level
...
def submit_report(request):
    ...
    # Update task status
    if task.status != 'wait_for_review':
        task.status = 'wait_for_review'
        task.save()
...
def get_my_tasks(request):
    ...
    for task in tasks:
        ...
        # Get site name from multiple possible sources
        if getattr(task, 'site_name', None):
            resolved_site_name = task.site_name
        elif getattr(task, 'site', None):
            resolved_site_name = task.site.site_name
        else:
            resolved_site_name = ''

        # Get lat/lon from site relation (if available)
        site_lat = None
        site_lon = None
        if getattr(task, 'site', None):
            site_lat = task.site.latitude if task.site and task.site.latitude else None
            site_lon = task.site.longitude if task.site and task.site.longitude else None

        # Alternative: Get lat/lon from SiteData using global_id (if site relation is not available)
        if (site_lat is None or site_lon is None) and task.global_id:
            try:
                site_data = SiteData.objects.get(global_id=task.global_id)
                site_lat = float(site_data.latitude) if site_data.latitude else None
                site_lon = float(site_data.longitude) if site_data.longitude else None
            except SiteData.DoesNotExist:
                pass  # Keep as None

        task_list.append({
            'task_id': task.task_id,
            'task_name': task.title if hasattr(task, 'title') else task.task_id,
            ...
            'reports': [
                ...
                for r in task.reports.filter(submitted_by=user)
            ] if hasattr(task, 'reports') else []
        })
...
def dashboard_stats(request):
    ...
    wait_for_review = Task.objects.filter(status='wait_for_review').count()

    data = {
        'total_tasks': total,
        'completed_tasks': completed,
        'pending_tasks': pending,
        'wait_for_review_tasks': wait_for_review,
    }
    return Response(data)